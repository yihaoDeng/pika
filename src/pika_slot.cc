...
void SlotKeyRem(const std::string key) {
    ...
    std::vector<std::string> members = {type + key};
    int64_t count;
    rocksdb::Status s = g_pika_server->db()->SRem(slotKey, type+key, &count);
    if (!s.ok()) {
        LOG(WARNING) << "Zrem key: " << key <<" from slotKey, error: " <<strerror(errno);
        return;
    }
...
void KeyNotExistsRem(const std::string type, const std::string key) {
    ...
    int64_t res = -1;
    std::vector<std::string> vkeys = {key};
    //vkeys.push_back(key);
    rocksdb::Status s = g_pika_server->db()->Exists(vkeys, &res);
    if (res == 0) {
        std::string slotKey = SlotKeyPrefix + std::to_string(SlotNum(key)); 
        rocksdb::Status s = g_pika_server->db()->SRem(slotKey, type+key, &res);
        if (!s.ok()) {
            LOG(WARNING) << "Zrem key: " << key <<" from slotKey, error: " <<strerror(errno);
            return;
        }
...
int KeyDelete(const std::string key, const char key_type){
    int64_t count = 0;
    rocksdb::Status s = g_pika_server->db()->DelSingleType(key, &count, key_type);
    if (!s.ok()) {
        if (s.IsNotFound()) {
...
static int migrateKeyTTl(pink::PinkCli *cli, const std::string key){
    ...
    pink::RedisCmdArgsType argv;
    std::string send_str;
    rocksdb::Status s = g_pika_server->db()->TTL(key, &ttl);
    if (s.ok() && (ttl > 0)){
...
static int listGetall(const std::string key, std::vector<nemo::IV> &ivs){
    rocksdb::Status s = g_pika_server->db()->LRange(key, 0, -1, ivs);
    if (!s.ok()) {
        if (s.IsNotFound()) {
...
static int zsetGetall(const std::string key, std::vector<nemo::SM> &sms){
    rocksdb::Status s = g_pika_server->db()->ZRange(key, 0, -1, sms);
    if (!s.ok()) {
        if (s.IsNotFound()) {
...
int SlotsMgrtTagSlotCmd::SlotKeyPop(){
    ...
    int64_t count;
    rocksdb::Status s = g_pika_server->db()->SRem(slotKey, tkey, &count);
    if (!s.ok()) {
...
void SlotsDelCmd::Do() {
    ...
    int64_t count = 0;
    rocksdb::Status s = g_pika_server->db()->MDel(keys, &count);
    if (s.ok()) {
...
bool SlotsMgrtSenderThread::ElectMigrateKeys(){
    ...
    for(; iter->Valid(); iter->Next()){
        ...
        rocksdb::Status s = g_pika_server->db()->SRem(slotKey, tkey, &remained);
        if (!s.ok()) {
            if (s.IsNotFound()) {
